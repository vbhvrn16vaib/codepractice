    ...
    void dfs(int[][] grid, int i , int j,Integer[][] dp){
        ...
        while(!que.isEmpty()){
            ...
            for(int[] dir : dirs){
                ...
                dp[x1][x2] = sf+1;
                que.add(new int[]{x1,x2,sf+1});
            }
        }
        grid[i][j] = 1;
    }
    
    boolean isNotValid(int[][] grid, int i, int j, boolean[][] visited){
        return i < 0 || j < 0 || i == grid.length || j == grid[0].length || grid[i][j] == 1 || grid[i][j] == 2 || visited[i][j];
    }
    
    void sum(Integer[][] dp, int[][] fdp){
        for(int i =0 ; i < dp.length; i++){
            for(int j = 0; j < dp[0].length; j++){
                if (fdp[i][j] == Integer.MAX_VALUE || dp[i][j] == null) {
                    fdp[i][j]=Integer.MAX_VALUE;
                    continue;
                }
                fdp[i][j]+=dp[i][j];
            }
